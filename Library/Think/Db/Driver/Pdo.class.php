...
class Pdo extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            _log("dbType:".$this->dbType, "connect", "Pdo");
    ...
    public function getFields($tableName) {
        $this->initConnect(true);
        _log("dbType=".$this->dbType,"getFields","Pdo");
     ...
     * @access public
     */
    public function close($release = false) {
      if($release && C('CONNECT_POOL') === false){
        return;
      }else if($release && C('CONNECT_POOL') === true){
        if($this->_linkID) $this->_linkID->release();
    ...
    public function escapeString($str) {
       _log("type:".$this->dbType, "escapeString", "Pdo", "BIBI");
         switch($this->dbType) {
            case 'PGSQL':                
            case 'IBASE':                
            case 'SQLITE':
    ...
    protected function parseValue($value) {
      _log("dbType:".$this->dbType, "parseValue", "Pdo", "DEBUG");
        if(is_string($value)) {
            $value =  strpos($value,':') === 0 ? $this->escapeString($value) : '\''.$this->escapeString($value).'\'';
            _log("value is string:".$value, "parseValue", "Pdo", "DEBUG");
         ...
         _log("value:".$value, "parseValue", "Pdo", "DEBUG");
        return $value;
    }
