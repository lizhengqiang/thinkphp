...
class Db {
    ...
    public static function getInstance($db_config='') {
  		...
  		_log("db_config:".json_encode($db_config), "getInstance", "Db");
  		_log("guid:".$guid, "getInstance", "Db");
  		if(!isset($_instance[$guid])){
  			$obj	=	new Db();
  			$_instance[$guid]	=	$obj->factory($db_config);
  			_log("factory:".$guid, "getInstance", "Db");
  		}
  		_log("_instace:".json_encode($_instance), "getInstance", "Db");
    ...
    public function factory($db_config='') {
        ...
        _log("db_config:".json_encode($db_config), "factory", "Db");
    ...
    protected function parseValue($value) {
      _log("value", "parseValue", "Db");
        ...
        _log("value:".$value, "parseValue", "Db");
        return $value;
    }
    ...
    protected function parseWhere($where) {
        ...
        if(is_string($where)) {
            ...
            foreach ($where as $key=>$val){
              _log("foreach:".json_encode($where), "parseWhere", "Db");
                $whereStr .= '( ';
                if(is_numeric($key)){
                    $key  = '_complex';
                }                    
    ...
    protected function parseWhereItem($key,$val) {
        $whereStr = '';
        _log("val:".json_encode($val), "parseWhereItem", "Db");
        if(is_array($val)) {
            if(is_string($val[0])) {
          ...
          _log("parseValue,dbType=".$this->dbType, "parseWhereItem", "Db");
            //对字符串类型字段采用模糊匹配
        ...
        _log("whereStr:".$whereStr, "parseWhereItem", "Db");
        return $whereStr;
    }
    ...
    public function buildSelectSql($options=array()) {
        ...
        _log('DB_SQL_BUILD_CACHE:'.json_encode(C('DB_SQL_BUILD_CACHE')), 'buildSelectSql', 'Db');
        if(C('DB_SQL_BUILD_CACHE')) { // SQL创建缓存
            $key    =  md5(serialize($options));
            $value  =  S($key);
            _log(json_encode($options).'=>'.json_encode($value), 'buildSelectSql', 'Db');
            if(false !== $value) {
              _log(json_encode($options).'=>'.json_encode($value), 'buildSelectSql', 'Db');
                return $value;
            }
        ...
        _log('sql:'.$sql, 'buildSelectSql', 'Db');
        $sql .=     $this->parseLock(isset($options['lock'])?$options['lock']:false);
        _log('sql:'.$sql.',dbType='.$this->dbType, 'buildSelectSql', 'Db');
    ...
    public function parseSql($sql,$options=array()){
        ...
        _log('sql:'.$sql.',dbType='.$this->dbType, 'parseSql', 'Db');
        return $sql;
    }
