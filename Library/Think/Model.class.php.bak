...
class Model {
        ...
        if(empty($this->fields)) {
            ...
            if(C('DB_FIELDS_CACHE')) {
                ...
                if($fields) {
                    $this->fields   =   $fields;
                    $this->pk       =   $fields['_pk'];
                    _log($fields, '_checkTableInfo', 'Model');
                    return ;
                }
            }
            // 每次都会读取数据表信息
    ...
    public function find($options=array()) {
      _log("find:".json_encode($options), "find", "Model");
        if(is_numeric($options) || is_string($options)) {
        ...
        _log($options, "find", "Model");
        _log("dbType=".$this->db->dbType, "find", "Model");
        $resultSet          =   $this->db->select($options);
        _log($resultSet);
        if(false === $resultSet) {
            return false;
